...
<div class="row hidden-print">
    <div class="col-lg-12">
        {!!Form::submit('Generar',['class'=>'btn btn-primary', 'id' => ''])!!}
    </div>
</div>
{!!Form::close()!!}
@php
...
@endphp
@if($visible)
<div class="row" style="margin-top: 50px; background: #E9F7EF">
    <div class="col-md-12">
...
<div class="row" style="margin-top: 50px; page-break-after: always !important;">
    ...
    <div class="col-md-6" id="box5" style="page-break-inside: avoid;">
        <!-- AREA CHART -->
        <div class="box box-primary">
            <div class="box-header with-border">
...
<div class="col-md-6" style="page-break-inside: avoid;">
    <!-- AREA CHART -->
    <div class="box box-primary">
        <div class="box-header with-border">
	...
	<script>
		...
		$('#granja').change(function(event){
			...
			if ($('#granja').val() > 0) {
				...
				$.get(route+'/'+empresa+'/'+granja,function(response, state){
					$('#lote').empty();
					//console.log(response.length);
...
//params
            ...
            $edad = DB::table('animal_grupo_linea_peso')
                    ->where('fk_linea_id', '=', 1)
                    ->select('peso_unidad')
                    ->first();
            ...
            $IsacTotal = EstudioGalponResultados::select(
                            'estudio_galpon_resultados.lote', 'estudio_galpones.fk_estudio_id', 'estudio_galpones.individuos_semanas', 'empresa_granja_galpones.galpon_nombre'/* esto es lo que copias */, \DB::raw('100*(1-SUM(estudio_galpon_resultados.isag)) as isagtotal')
                    )
                    ->join('estudio_galpones', 'estudio_galpon_resultados.fk_estudio_galpon_id', 'estudio_galpones.estudio_galpon_id')
                    ->join('empresa_granja_galpones', 'estudio_galpones.fk_galpone_id', 'empresa_granja_galpones.galpone_id')//copiar entero
                    // ->leftJoin('animal_linea','estudio_galpon_resultados.fk_linea_id','animal_linea.linea_id')
                    ->where([
                    ...
                    ->orderBy('estudio_galpones.individuos_semanas')->get();



            // Unidades de la Edad para pesos

            $edad = EstudioGalponResultados::select(
                            'animal_grupo_linea_peso.peso_unidad'
                    )
                    ->join('estudio_galpones', 'estudio_galpon_resultados.fk_estudio_galpon_id', 'estudio_galpones.estudio_galpon_id')
                    ->join('empresa_granja_galpones', 'estudio_galpones.fk_galpone_id', 'empresa_granja_galpones.galpone_id')//copiar entero
                    // ->leftJoin('animal_linea','estudio_galpon_resultados.fk_linea_id','animal_linea.linea_id')
                    ->join('animal_grupo_linea_peso', 'estudio_galpon_resultados.fk_linea_id', 'animal_grupo_linea_peso.fk_linea_id')
                    ->where([
                        'estudio_galpon_resultados.fk_empresa_id' => $company,
                        'estudio_galpon_resultados.fk_granja_id' => $granja,
                        //'estudio_galpon_resultados.fk_galpone_id' => $galpon,
            ...
            // dd($edad);
            //graphic 28
            $var28 = EstudioGalponResultados::select(
                            'variables.variable_id', 'variables.variable_nombre', 'estudio_galpon_resultados.lote', 'estudio_galpones.fk_estudio_id', 'estudio_galpones.individuos_semanas', 'estudio_galpon_resultados.resultado', 'empresa_granja_galpones.galpon_nombre'
                    )
                    ->join('estudio_galpones', 'estudio_galpon_resultados.fk_estudio_galpon_id', 'estudio_galpones.estudio_galpon_id')
                    ->join('variables', 'estudio_galpon_resultados.fk_variable_id', 'variables.variable_id')
                    ->join('empresa_granja_galpones', 'estudio_galpones.fk_galpone_id', 'empresa_granja_galpones.galpone_id')
                    ->where([
                        'estudio_galpon_resultados.fk_empresa_id' => $company,
                        'estudio_galpon_resultados.fk_granja_id' => $granja,
                        //'estudio_galpon_resultados.fk_galpone_id' => $galpon,
                    ...
                    ->orderBy('estudio_galpones.individuos_semanas')->get();
            //postura
            $tres_variables = estudioGalpones::select(
                            'estudio_galpones.postura', 'estudio_galpones.consumo', 'estudio_galpones.huevos','estudio_galpones.individuos_semanas','empresa_granja_galpones.galpon_nombre'
                    )
                    ->join('empresa_granja_galpones', 'estudio_galpones.fk_galpone_id', 'empresa_granja_galpones.galpone_id')
                    ->where([
                        'estudio_galpones.fk_empresa_id' => $company,
                        'estudio_galpones.fk_granja_id' => $granja,
                        'estudio_galpones.estudio_galpon_lote' => $lote,
                    ])
                    ->orderBy('estudio_galpones.individuos_semanas')->get();
...
//            dd($tres_variables);
            ...
            $variable3 = EstudioGalponResultados::select(
                            'variables.variable_id', 'variables.variable_nombre', 'estudio_galpon_resultados.lote', 'estudio_galpones.fk_estudio_id', 'estudio_galpones.individuos_semanas', 'empresa_granja_galpones.galpon_nombre', \DB::raw('100*estudio_galpon_resultados.porcentaje_afectacion as porcentaje_afectacion')
                    )
                    ->join('estudio_galpones', 'estudio_galpon_resultados.fk_estudio_galpon_id', 'estudio_galpones.estudio_galpon_id')
                    ->join('variables', 'estudio_galpon_resultados.fk_variable_id', 'variables.variable_id')
                    ->join('empresa_granja_galpones', 'estudio_galpones.fk_galpone_id', 'empresa_granja_galpones.galpone_id')
		...
		@if($visible)
		    	...
		    	labels: [
		    		@php
		        		foreach($IsacTotal as $isac){
		        		               ...
		        		               if($edad->peso_unidad=="Semanas"){

		        		               	  print_r('["Semanas '.$isac->individuos_semanas.'"'.",".'"'.$isac->galpon_nombre.'"],');

		        		               }if ($edad->peso_unidad=="Dias"){
                  
                                          print_r('["Dias '.$isac->individuos_semanas.'"'.",".'"'.$isac->galpon_nombre.'"],');
		        		               } 
		        		...
		        		foreach($variable1 as $cm){
		        			//echo '"Semana '.$cm->individuos_semanas.'",';		        			
		        		               ...
		        		               if($edad->peso_unidad=="Semanas"){
		        		               	  ...
		        		               	  print_r('["Semanas '.$cm->individuos_semanas.'"'.",".'"'.$cm->galpon_nombre.'"],');

		        		               }if ($edad->peso_unidad=="Dias"){
                  
                                          print_r('["Dias '.$cm->individuos_semanas.'"'.",".'"'.$cm->galpon_nombre.'"],');
		        		               }
		        		}
		        	@endphp
			    ],
          ...
          var ctx = document.getElementById("Chartvar27").getContext('2d');
			...
			data = {
			    datasets: [
			     	{
		                label: "CONSUMO DE ALIMENTO G/DÍA",
		                data: [
		                	@php
				        		foreach($tres_variables as $cm){
				        			echo '"'.$cm->consumo.'",';
